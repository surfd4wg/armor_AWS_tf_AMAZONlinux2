This is a Terraform script that automatically builds an Amazon Linux 2 Server in AWS EC2. 
It also installs various programs through a EC2 user data shell script. 
The various programs are: -Armor Agent 3.0 -Chrome -Apache2 -User + Password for Remote Desktop

Change the ARMOR License Key in the install_userdata_amazonix.sh file.
...
ssh -i ~/.ssh/(key file).pem ec2-user@ec2-X-X-X-X.compute-1.amazonaws.com

If you want to remote desktop into the machine you will need to install amazon linux desktop. 
